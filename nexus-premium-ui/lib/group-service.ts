...
/**
 * Lists only the signed-in user's groups.
 *
 * A successful empty array means the user genuinely has no groups. Backend
 * failures are thrown so consumers can render an error state instead of
 * mistaking infrastructure failure for an empty account.
 */
export async function listMyGroups(): Promise<GroupSummary[]> {
  ...
  if (error) {
    ...
    throw new Error(`Unable to load your groups: ${message}`)
  }
