...
function score(v: PlannerVenue, radiusKm: number, budget: BudgetPreference) { const dist = Math.round((1 - Math.min(v.distanceFromCentre / radiusKm, 1)) * 40); const target = budget === 'low' ? 1 : budget === 'high' ? 3 : 2; const price = v.priceLevelKnown === false ? 8 : Math.max(0, 20 - Math.abs(v.priceLevel - target) * 7); return Math.min(100, dist + price + (v.openingHoursKnown ? 20 : 10) + (v.isRealData ? 10 : 0)) }
function routeOrder(venues: PlannerVenue[]) { const remaining = [...venues]; const result: PlannerVenue[] = []; if (remaining.length) result.push(remaining.shift()!); while (remaining.length) { const last = result[result.length - 1]!; let best = 0, bestDist = Infinity; remaining.forEach((v, i) => { const d = distance(last, v); if (d < bestDist) { best = i; bestDist = d } }); result.push(remaining.splice(best, 1)[0]!) } return result }

export const pubCrawlPlannerV2: PlannerDefinition = {
  id: 'pub-crawl-planner-v2', activityId: 'pub-crawl', kind: 'venue', name: 'Pub Crawl Planner',
  ...
  async plan(request: PlannerRequest): Promise<PlannerResult> {
    ...
    const venues = await getUniversalOsmVenues('pub-crawl', request.groupLocation, radius)
    ...
    const ordered = routeOrder(ranked.slice(0, Math.min(request.desiredStops ?? 4, ranked.length)).map(x => x.venue))
    ...
    const stops: PlannerStop[] = ordered.map((venue, index) => {
      ...
      const arrival = current, departure = add(arrival, STOP_MINUTES); current = departure
      return { order: index + 1, venue, arrivalTime: arrival, departureTime: departure, walkingFromPrevious: walk, distanceFromPrevious: previous ? Math.round(distance(previous, venue) * 100) / 100 : 0, score: { total: score(venue, radius / 1000, budget), breakdown: { rating: 0, distance: 0, price: 0, atmosphere: 0, openingHours: 0, capacity: 0 } }, role: index === 0 ? 'Opener' : index === ordered.length - 1 ? 'Finale' : 'Stop', reason: venue.openingHoursKnown ? 'Real venue with listed opening hours' : 'Real venue — opening hours need verification' }
    })
    ...
    return { kind: 'venue', title: '🍺 Pub Crawl', subtitle: `${DAY_LABELS[request.goldenWindow.day_of_week] ?? 'Selected day'} · ${fmt(start)}`, activityId: 'pub-crawl', durationMinutes: ordered.length * STOP_MINUTES + walkingMinutes, estimatedCostLabel: '££', totalDistanceKm, walkingMinutes, stops, overallScore: Math.round(stops.reduce((sum, s) => sum + s.score.total, 0) / stops.length), explanation: `Built a ${ordered.length}-stop route from ${venues.length} real pubs and bars near ${request.locationName ?? 'your planning location'}.`, warnings, generatedAt: new Date().toISOString(), goldenWindowQuality: quality, groupMatchPercent: matchPercent, dataSource: 'real', providerName: 'OpenStreetMap', scoreReasons: ['Real nearby venues', 'Route ordered to reduce walking between stops'] }
  },
}
