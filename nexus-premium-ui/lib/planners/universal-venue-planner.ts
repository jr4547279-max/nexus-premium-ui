...
function scoreVenue(venue: PlannerVenue, startTime: string, radiusKm: number, budget: BudgetPreference) {
  ...
  const data = venue.isRealData ? 10 : 0
  return Math.min(100, distance + hours + price + data)
}

export interface UniversalVenueConfig {
  activityId: string
...
export function createUniversalVenuePlanner(config: UniversalVenueConfig): PlannerDefinition {
  return {
    id: `${config.activityId}-universal-planner`,
    activityId: config.activityId,
    kind: 'venue',
    name: `${config.activityLabel} Planner`,
    description: `Finds real ${config.activityLabel.toLowerCase()} locations near the group's planning location.`,
    async plan(request: PlannerRequest): Promise<PlannerResult> {
      ...
      const venues = await provider.getVenues(config.activityId, request.groupLocation)
      if (!venues.length) {
        throw new Error(`No real ${config.activityLabel.toLowerCase()} locations were found within ${Math.round(radiusMetres / 100) / 10} km. Try moving the planning location or widening the search area.`)
      }

      const start = request.goldenWindow.start_time
      const budget = request.budgetPreference ?? 'medium'
      const ranked = venues
        .map(venue => ({ venue, score: scoreVenue(venue, start, radiusMetres / 1000, budget) }))
      ...
      const stop: PlannerStop = {
        ...
        venue: best.venue,
        arrivalTime: start,
        departureTime: addMinutes(start, request.goldenWindow.duration_minutes),
        walkingFromPrevious: 0,
        distanceFromPrevious: 0,
        score: {
          total: best.score,
          breakdown: { rating: 0, distance: Math.min(20, Math.round(best.venue.distanceFromCentre < radiusMetres / 1000 ? 20 : 0)), price: 0, atmosphere: 0, openingHours: 0, capacity: 0 },
        },
      }

      const quality = (request.goldenWindow.match_quality ?? 'partial') as MatchQuality
      const matchPercent = request.goldenWindow.available_member_count != null && request.goldenWindow.total_member_count
        ? Math.round((request.goldenWindow.available_member_count / request.goldenWindow.total_member_count) * 100)
        : undefined
      ...
      return {
        kind: 'venue',
        title: `${config.emoji} ${config.activityLabel}`,
        subtitle: `${DAY_LABELS[request.goldenWindow.day_of_week] ?? 'Selected day'} · ${format12h(start)}`,
        activityId: config.activityId,
        durationMinutes: request.goldenWindow.duration_minutes,
        estimatedCostLabel: best.venue.priceLevelKnown === false ? 'Price unknown' : '£'.repeat(Math.max(1, best.venue.priceLevel)),
        totalDistanceKm: best.venue.distanceFromCentre,
        walkingMinutes: 0,
        stops: [stop],
        ...
        goldenWindowQuality: quality,
        groupMatchPercent: matchPercent,
        dataSource: 'real',
        providerName: 'OpenStreetMap',
        scoreReasons: [
          `${Math.round(best.venue.distanceFromCentre * 100) / 100} km from your planning point`,
          best.venue.openingHoursKnown ? 'Opening hours are listed' : 'Opening hours need verification',
        ],
      }
    },
  }
