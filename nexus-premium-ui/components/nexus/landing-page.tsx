  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden">
      <OrbitalBackground className="min-h-screen">
        {/* Header — interactive, sits above decorative layers */}
        ...
        {/* Hero Section — interactive, sits above decorative layers */}
        ...
        <footer className="relative z-10 px-5 py-6 border-t border-border/30">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
            ...
            <p className="text-muted-foreground text-xs">2024 Nexus</p>
          </div>
        </footer>
      </OrbitalBackground>
    </div>
...
function FeatureIcon({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="w-9 h-9 rounded-lg bg-muted/50 flex items-center justify-center text-muted-foreground pointer-events-none">
        {icon}
      </div>
      <span className="text-[10px] text-muted-foreground text-center leading-tight max-w-[60px] pointer-events-none">
        {label}
      </span>
    </div>
  )
    ...
    <div className="grid md:grid-cols-2 gap-6 items-center">
      <GlassCard className="p-4">
        <div className="flex items-start gap-3 mb-4">
          <div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center shrink-0 pointer-events-none">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-muted-foreground text-xs">We only read availability. Events stay private.</p>
          </div>
        </div>
        <Button variant="secondary" className="w-full justify-start gap-2.5 h-10 text-sm">
        ...
        <p className="text-[10px] text-muted-foreground text-center mt-3">More options coming soon</p>
      </GlassCard>

      {/* Orbital calendar visualisation — purely decorative */}
      <div className="relative h-64 hidden md:flex items-center justify-center pointer-events-none" aria-hidden="true">
...
function FeaturesSection() {
  const features = [
    { icon: <Calendar className="w-5 h-5" />, title: 'Calendars', description: 'Find when everyone is free.' },
    { icon: <Users className="w-5 h-5" />, title: 'Preferences', description: 'Factor in food, budget & more.' },
    { icon: <MapPin className="w-5 h-5" />, title: 'Location', description: 'Choose spots that work for all.' },
    { icon: <Sparkles className="w-5 h-5" />, title: 'AI Magic', description: 'We do the heavy lifting.' },
  ]
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {features.map((f) => (
        <GlassCard key={f.title} className="text-center p-4">
          <div className="w-10 h-10 rounded-lg bg-muted/50 flex items-center justify-center mx-auto mb-3 text-muted-foreground pointer-events-none">
            {f.icon}
          </div>
