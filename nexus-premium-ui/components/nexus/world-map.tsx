...
export function WorldMap({ onNavigate: _onNavigate }: WorldMapProps) {
  ...
  const getUserLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      mapRef.current?.flyTo({ center: [next.lng, next.lat], zoom: 13, pitch: 38, bearing: 0, duration: 1400, essential: true })
  ...
  useEffect(() => {
    ...
    import('maplibre-gl').then((maplibregl) => {
      ...
      // Next.js/Turbopack needs a same-origin worker. The build places both
      // MapLibre worker modules in /public/maplibre before Netlify builds.
      ...
      try {
        ...
        try { map.setProjection({ type: 'globe' }) } catch { /* supported progressively */ }
        ...
        map.on('load', () => {
          ...
          // OpenFreeMap exposes the OpenMapTiles planet source. We add our own
          // extrusion layer so the street-level reveal has real building mass.
          if (!map.getSource('nexus-buildings')) {
            map.addSource('nexus-buildings', { type: 'vector', url: 'https://tiles.openfreemap.org/planet' })
          }
            ...
            map.addLayer({
              ...
              paint: {
                'fill-extrusion-color': [
                  ...
                  14, '#111a29', 17, '#1b2a40', 20, '#263a55',
                ],
                'fill-extrusion-opacity': 0.94,
                'fill-extrusion-height': [
                  'interpolate', ['linear'], ['zoom'],
                  14, 0,
                  16, ['coalesce', ['get', 'render_height'], 9],
                  20, ['coalesce', ['get', 'render_height'], 12],
                ],
        ...
        map.on('error', (event) => {
          ...
          if (!mapReady) setMapError('Map tiles are taking longer than expected. Retrying…')
        })
      } catch (error) {
  ...
  useEffect(() => {
    ...
    venues.forEach((venue) => {
      ...
      el.onclick = (event) => {
        ...
        map.stop()
        map.flyTo({
          center: [venue.lng as number, venue.lat as number],
          zoom: 17.2,
          pitch: 66,
          bearing: -18,
          duration: 1850,
  ...
  const countLabel = useMemo(
    () => `${venues.length} ${VIBE_LABEL[vibe].toLowerCase()} ${venues.length === 1 ? 'spot' : 'spots'}`,
    [venues.length, vibe],
  )
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#02040a]">
      ...
      <div className="absolute bottom-24 left-4 right-4 z-20 flex items-end justify-between gap-3 pointer-events-none">
        <div className="pointer-events-auto rounded-2xl border border-white/10 bg-black/45 px-3 py-2.5 backdrop-blur-xl">
          <div className="flex items-center gap-2 text-[10px] text-white/55"><span className="h-2 w-2 rounded-full bg-amber-300 shadow-[0_0_10px_#fbbf24]" />Live venues<span className="text-white/20">·</span><span className="inline-flex items-center gap-1"><Navigation className="h-3 w-3" /> 3D view</span></div>
        </div>
